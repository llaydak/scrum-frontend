...
export default function Dashboard(props) {
  ...
  const processBurndownData = (data) => {
    if (!data || !data.changes || !data.startTime)
      return [];

    const { startTime, endTime, changes } = data;
    let allTimestamps = Object.keys(changes).map(Number);
    allTimestamps.push(startTime);
    allTimestamps.push(endTime);
    allTimestamps = [...new Set(allTimestamps)].sort((a, b) => a - b);
    const issueState = {};
    const chartData = [];
    let sprintStartScope = 0; 

    allTimestamps.forEach((ts) => {
      if (changes[ts]) {
        changes[ts].forEach((change) => {
          const key = change.key;

          if (!issueState[key]) {
            issueState[key] = { sp: 0, done: false };
          }

          if (change.statC) {
            issueState[key].sp = change.statC.newValue || 0;
          }

          if (change.column) {

            if (change.column.done !== undefined) {
              issueState[key].done = change.column.done;
            } else if (change.column.notDone !== undefined) {
              issueState[key].done = !change.column.notDone;
            }
          }
      ...
      Object.values(issueState).forEach((issue) => {
        if (!issue.done) {
          currentRemaining += issue.sp;
        }
      });
      if (ts >= startTime && sprintStartScope === 0) {
        sprintStartScope = currentRemaining;
      }
    ...
    const finalChartData = chartData.filter(
    ...
    finalChartData.forEach((point) => {
      if (point.timestamp >= startTime) {
        const timePassed = point.timestamp - startTime;
        
        let ideal =
          sprintStartScope - (sprintStartScope / totalDuration) * timePassed;
        point.ideal = Math.max(0, Math.round(ideal));
      }
    });
    console.log("İşlenmiş Burndown Veri:", finalChartData);
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8080/burndown-greenhopper")
      .then((response) => {
        const processedData = processBurndownData(response.data);
        setBurndownData(processedData);
      })
      .catch((error) => {
        console.error("Burndown verisi alınamadı:", error);
      });
    axios
